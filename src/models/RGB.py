...
class spatial_network(Module):
    ...
    def __init__(self, bands, classes):
        ...
        self.HSI_conv1 = conv_module(in_channels=bands, filters=32)
        self.RGB_conv1 = conv_module(in_channels=3, filters=32)        
        self.attention_1 = spatial_attention(filters=32, classes = classes)
        
        # Second block is 64 filters
        self.HSI_conv2 = conv_module(in_channels=32, filters=64, maxpool_kernel=(2,2))
        self.attention_2 = spatial_attention(filters=64, classes = classes)
    
        # Second block is 128 filters
        self.HSI_conv3 = conv_module(in_channels=64, filters=128, maxpool_kernel=(2,2))
    ...
    def forward(self, hsi, rgb):
        ...
        x = self.HSI_conv1(hsi)
        rgb_map = self.RGB_conv1(rgb)
        ...
        x = self.HSI_conv2(x, pool = True)
        x, scores2 = self.attention_2(x)
        
        x = self.HSI_conv3(x, pool = True)        
...
class Hang2020(Module):
    ...
    def forward(self, hsi, rgb):
        spectral_scores = self.spectral_network(hsi)
        spatial_scores = self.spatial_network(hsi, rgb)
